...
void IOTAppStory::firstBoot(bool ea)
{
    ...
    String(_appVersion).toCharArray(config.compDate, 20);
    writeConfig();
...
void IOTAppStory::begin(int feedBackLed, bool bootstats, bool ea)
{
    ...
    // make sure the timedifference during startup is bigger than 10 sec. Otherwise it will go either in config mode or calls home
    buttonEntry = millis() + 10000;
...
{
    String configVersion(config.compDate);
    int configMainVersionEnd = configVersion.lastIndexOf(".");
    if (configMainVersionEnd == -1)
        return true;
    
    int mainVersionEnd = _appVersion.lastIndexOf(".");
