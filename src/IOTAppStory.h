...
#define STRUCT_CHAR_ARRAY_SIZE 50  // length of config variables
  ...
  class IOTAppStory {
    public:
	  ...
	  // function for setting "Wifi" config parameters ssid & password
	 
	  void preSetConfig(String ssid, String password);
	  void preSetConfig(String ssid, String password, bool automaticUpdate);
	  
	  // function for setting config parameters ssid, password & boardName
	  ...
	  // function for setting all config parameters except IOTappStory2 & IOTappStoryPHP1
      ...
      //void boot(struct config* config,int feedBackLed=-1, bool bootstats=true);
      void boot(void(*ptr)(void), int feedBackLed=-1, bool bootstats=true);
	  ...
	  void useXfields(const int nrof);
	  
	  void addField(const char *fieldIdName,const char *fieldLabel,const char *varPointer, int length);
