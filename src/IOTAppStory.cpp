...
#define ENTER_CONFIG_MODE_TIME_MAX 15000
...
IOTAppStory::IOTAppStory(const char *appName, const char *appVersion, const char *compVersion, const int modeButton){
	_appName = appName;
	_appVersion = appVersion;
	_firmware = String(appName) + String(appVersion);
	_compVersion = compVersion;
...
void IOTAppStory::firstBoot(bool ea){
	...
	String(_appVersion).toCharArray(config.compVersion, 20);
	writeConfig();
...
void IOTAppStory::begin(bool bootstats, bool ea){
	...
	// make sure the time difference during startup is bigger than 10 sec. 
	// Otherwise it will go either in config mode or calls home
	buttonEntry = millis() + ENTER_CONFIG_MODE_TIME_MAX;
	buttonStateOld = HIGH;
...
void IOTAppStory::connectNetwork() {
	...
	if (!isNetworkConnected()) {
		...
		// if password forgotten by firmwware try again with stored PW
		WiFi.begin(config.ssid, config.password);
		if (!isNetworkConnected())
			espRestart('C', "Going into Configuration Mode"); // still no success
	}
	DEBUG_PRINTLN(" ");
...
byte IOTAppStory::iotUpdaterSPIFFS(String server, String url, String firmware, bool immediately) {
	...
	switch (retspiffs) {
		...
		case HTTP_UPDATE_NO_UPDATES:
			DEBUG_PRINTLN(" SPIFFS No updates");
			retValue = 'A';
		break;

		case HTTP_UPDATE_OK:
			DEBUG_PRINTLN(" SPIFFS Received update");
			retValue = 'U';
		break;
	}
	return retValue;
...
void IOTAppStory::initWiFiManager() {
	...
	if (WiFi.SSID() == "") {
		...
		// Force to station mode because if device was switched off while in access point
		// mode it will start up next time in access point mode.
		WiFi.mode(WIFI_STA);
...
void IOTAppStory::espRestart(char mmode, char* message) {
	//LEDswitch(GreenFastBlink);
	while (digitalRead(_modeButton) == LOW)
...
void IOTAppStory::eraseFlash(unsigned int eepFrom, unsigned int eepTo)
{
	...
	EEPROM.begin(EEPROM_SIZE);
	for (unsigned int t = eepFrom; t < eepTo; t++) {
		EEPROM.write(t, 0);
	}
	EEPROM.end();
}
...
bool IOTAppStory::readConfig() {
	...
	if (EEPROM.read(magicBytesBegin) == MAGICBYTES[0] &&
		EEPROM.read(magicBytesBegin + 1) == MAGICBYTES[1] &&
		EEPROM.read(magicBytesBegin + 2) == MAGICBYTES[2]) {
...
void IOTAppStory::routine() {
	...
	int buttonState = digitalRead(_modeButton);
	...
	if (buttonStateOld != buttonState) {
		...
		if (!isModeButtonPressed()) {
			switch(getModeButtonStatus(false)) {
	...
	if (isModeButtonPressed() && millis() - debugEntry > 1000) {
		debugEntry = millis();
		
		switch(getModeButtonStatus()) {
		case MODE_BUTTON_LONG_PRESS:
			DEBUG_PRINTLN("Button Press: Config Mode");
			break;
		
		case MODE_BUTTON_SHORT_PRESS:
			DEBUG_PRINTLN("Button Press: Check Firmware");
			break;
		
		default:
			DEBUG_PRINTLN("Button Press: Nothing to do");
		}
	}
...
int IOTAppStory::getModeButtonStatus(bool checkCurrentButtonState /*= true*/) {
	if (checkCurrentButtonState && !isModeButtonPressed())
		return MODE_BUTTON_NO_PRESS;
	
	unsigned long buttonTime = millis() - buttonEntry;
	
	if (buttonTime >  ENTER_CONFIG_MODE_TIME_MIN && 
		buttonTime <= ENTER_CONFIG_MODE_TIME_MAX) {
		return MODE_BUTTON_LONG_PRESS;
	}
	if (buttonTime >  ENTER_CHECK_FIRMWARE_TIME_MIN && 
		buttonTime <= ENTER_CHECK_FIRMWARE_TIME_MAX) {
		return MODE_BUTTON_SHORT_PRESS;
	}
	
	return MODE_BUTTON_NO_PRESS;
}

bool IOTAppStory::isFirstBoot() {
	String configVersion(config.compVersion);
	int configMainVersionEnd = configVersion.indexOf(".");
	if (configMainVersionEnd == -1)
		return true;
	
	int mainVersionEnd = _appVersion.indexOf(".");
	if (mainVersionEnd != configMainVersionEnd)
		return true;
	
	for(int i = 0; i < configMainVersionEnd; i++) {
		if (configVersion[i] != _appVersion[i])
			return true;
	}
