	...
	*///---------------------------------------------------------------------------
	String WiFiConnector::getSSIDfromEEPROM(const int apNr) {
		EEPROM.begin(EEPROM_SIZE);
		...
		#if WIFICONNECTOR_DEBUG == true
			Serial.print(F("WIFICONNECTOR_DEBUG\t| Returning getSSIDfromEEPROM("));
			Serial.print(apNr);
			Serial.print(F(") "));
			Serial.println(ssid);
		#endif
	...
	*///---------------------------------------------------------------------------
	bool WiFiConnector::connectToAP(const char *waitChar) {
		if(this->_nrOfCfgFound == 0){
			#if WIFICONNECTOR_DEBUG == true
				Serial.print(F("WIFICONNECTOR_DEBUG\t| No WiFi creds found "));
			#endif
